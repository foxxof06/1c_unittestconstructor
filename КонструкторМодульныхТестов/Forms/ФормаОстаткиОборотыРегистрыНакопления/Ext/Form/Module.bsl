...
Процедура ЗаполнитьИсходныеДанные()
	...
	Если Параметры.Хранилище <> Неопределено Тогда
		...
		Если Параметры.РежимОстатков = Ложь Тогда
			
			ПериодОборотов = Новый СтандартныйПериод(ДляЗаполнения.Параметры.НачалоПериода, ДляЗаполнения.Параметры.КонецПериода);
	...
	Для каждого Ресурс из РегистрМетаданные.Ресурсы Цикл
		...
		НоваяСтрока.Использовать = Ложь;
		НоваяСтрока.Ресурс = Ресурс.Имя + ?(Параметры.РежимОстатков, "Остаток", "Оборот");
