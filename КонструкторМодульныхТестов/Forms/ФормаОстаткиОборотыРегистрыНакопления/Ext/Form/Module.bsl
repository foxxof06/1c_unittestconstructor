...
Процедура ЗаполнитьИсходныеДанные()
	...
	Измерения.Очистить();
	Ресурсы.Очистить();
	ТаблицаИзмеренияДляОтбора.Очистить();
	...
	Для каждого Измерение из РегистрМетаданные.Измерения Цикл		
		...
		НоваяСтрока = ТаблицаИзмеренияДляОтбора.Добавить();
		НоваяСтрока.ИмяИзмерения = Измерение.Имя;
		НоваяСтрока.Использовать = Истина;
	КонецЦикла;
	
	Для каждого Ресурс из РегистрМетаданные.Ресурсы Цикл		
		ТаблицаДанных.Колонки.Добавить(Ресурс.Имя + ПостфиксРесурса, Ресурс.Тип, Ресурс.Синоним);   		
		Ресурсы.Добавить(Ресурс.Имя + ПостфиксРесурса)
	КонецЦикла; 		
	...
	Если Параметры.Хранилище <> Неопределено Тогда
		...
		Если ДляЗаполнения.Свойство("ИзмеренияДляОтбора") Тогда
			
			Отбор = Новый Структура;
			Отбор.Вставить("ИмяИзмерения");
			
			Для Каждого СтрокаИзмеренияДляОтбора Из ТаблицаИзмеренияДляОтбора Цикл
				СтрокаИзмеренияДляОтбора.Использовать = Ложь;			
			КонецЦикла;

			
			Для Каждого ИмяИзмеренияОтбора Из ДляЗаполнения.ИзмеренияДляОтбора Цикл
													
				Отбор.ИмяИзмерения = ИмяИзмеренияОтбора;
				НайденныеСтроки = ТаблицаИзмеренияДляОтбора.НайтиСтроки(Отбор);
...
Процедура СгенерироватьТаблицуФормы(ТаблицаДанных)
	...
	Для Каждого Колонка Из ТаблицаДанных.Колонки Цикл
        ...
        НовыйЭлемент = Элементы.Добавить("ТаблицаРегистра" + Колонка.Имя, Тип("ПолеФормы"), ТаблицаПолейВыбора);
...
Процедура Команда1НаСервере()
	...
	МассивИзмеренияДляОтбора = Новый Массив;
	МассивКолонокКВыгрузке = Новый Массив;

	Для Каждого СтрокаТаблицы Из ТаблицаИзмеренияДляОтбора Цикл
		Если СтрокаТаблицы.Использовать Тогда
			МассивИзмеренияДляОтбора.Добавить(СтрокаТаблицы.ИмяИзмерения);	
		КонецЕсли;					
	КонецЦикла;
	       	       		     	
	МассивРесурсов = Ресурсы.ВыгрузитьЗначения();
	
	КолонкиИзмеренийДляВыгрузки = СтрСоединить(МассивИзмеренияДляОтбора, ", ");
	
	КолонкиИзмеренийДляВыгрузки = КолонкиИзмеренийДляВыгрузки + ", " + СтрСоединить(МассивРесурсов, ", ");
	
	МассивПортфелей = Новый Массив;
	
	Если МассивИзмеренияДляОтбора.Найти("Портфель") <> Неопределено Тогда 
		
		Для Каждого СтрокаТаблицы Из  ЭтотОбъект.ТаблицаРегистра Цикл
			
			Если МассивПортфелей.Найти(СтрокаТаблицы.Портфель) = Неопределено Тогда
				МассивПортфелей.Добавить(СтрокаТаблицы.Портфель);	
			КонецЕсли;
	...
	СтруктураСериализации.Вставить("Ресурсы", МассивРесурсов);
	СтруктураСериализации.Вставить("Измерения", Измерения.ВыгрузитьЗначения());
	СтруктураСериализации.Вставить("ИзмеренияДляОтбора", МассивИзмеренияДляОтбора);
	СтруктураСериализации.Вставить("Портфели", МассивПортфелей);
...
Функция ДанныеДляЗаполнения(ПараметрыЗаполнения)
	...
	Для Каждого Ресурс Из Ресурсы Цикл
		...
		МассивСтрокРесурсов.Добавить(
			СтрШаблон(ШаблонСтрокаЗапросаРесур, Ресурс.Значение));
...
&НаКлиенте
Процедура ИзмеренияДляОтбораИспользоватьПриИзменении(Элемент)
	
	ВидимостьКолонокТаблицыОтборов();
		
КонецПроцедуры

&НаКлиенте
Процедура ВидимостьКолонокТаблицыОтборов()
	
	Для Каждого КолонкаОтбора Из ЭтотОбъект.ТаблицаИзмеренияДляОтбора Цикл
		
		Элементы["ТаблицаРегистра" + КолонкаОтбора.ИмяИзмерения].Видимость = КолонкаОтбора.Использовать;
