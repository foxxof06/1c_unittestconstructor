	...
	Если ЭтоДокумент Или ЭтоСправочник Тогда
		...
		Если ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.ОперацияБух") Тогда
			...
			Для Каждого Движение Из ОбъектДокумента.Движения Цикл
				...
				МетаданныеРегистраДвижений = Движение.Метаданные();
				
				Ресурсы   = МетаданныеРегистраДвижений.Ресурсы;
				Измерения = МетаданныеРегистраДвижений.Измерения;
				Реквизиты = МетаданныеРегистраДвижений.Реквизиты;
				
				МассивМетаданных = Новый Массив;
				
				Если ТипЗнч(Движение) = Тип("РегистрБухгалтерииНаборЗаписей.Управленческий") Тогда
					
					МассивМетаданных.Добавить(Измерения);
					МассивМетаданных.Добавить(Реквизиты);
					
					Для каждого СтрокаНабора Из Движение Цикл
											
						Для каждого СтрокаНабора Из Движение Цикл
							
							Для Каждого Мета Из МассивМетаданных Цикл
								
								Для каждого Рекувизит Из Мета Цикл
									
									ЗначениеРеквизита = СтрокаНабора[Рекувизит.Имя];
									
									Если ЗначениеРеквизита <> СсылкаНаОбъект Тогда							
										ПолучитьОбъектыРекурсивно(ЗначениеРеквизита, МассивОбъектов, 0, 1); 														
									КонецЕсли;
							...
							Для каждого Субконто Из СтрокаНабора.СубконтоДт Цикл
								
								Если ЗначениеЗаполнено(Субконто.Значение) Тогда
									ПолучитьОбъектыРекурсивно(Субконто.Значение, МассивОбъектов, 0, 1);	
								КонецЕсли; 								
								
							КонецЦикла;
							
							Для каждого Субконто Из СтрокаНабора.СубконтоКт Цикл
...
Процедура ЗаполнитьТаблицуМетодовОбъектов(ИмяКонфигурации)
	...
	ШаблонИмяМакета = Метаданные.Имя + "_ПроцедурыФункцииОбъектов";
...
Процедура ЗаполнитьТаблицуИсключаемыхИзВыгрузкиОбъектов(ИмяКонфигурации)
	...
	ШаблонИмяМакета = Метаданные.Имя + "_ИсключаемыеИзВыгрузкиРеквизитыОбъектов";
