...
Процедура ПолучитьОбъектыРекурсивно(СсылкаНаОбъект, МассивОбъектов, ТекущийУровеньРекурсии, МаксУровеньРекурсии) 
	...
	Если ЭтоДокумент Или (ЭтоСправочник И Не СсылкаНаОбъект.Предопределенный) Или (ЭтоПланСчетов И Не СсылкаНаОбъект.Предопределенный) Тогда
...
Процедура ЗаполнитьТаблицуДоступныхКоманд()
	...
	НоваяСтрока.Представление = "Выполнить функцию";
	НоваяСтрока.ОграничениеТипа = "";
	НоваяСтрока.Вид = "Метод";
	НоваяСтрока.ТипУзла = "Независимый";
...
Процедура СгенерироватьТекстыМодуля()
	...
	Для Каждого Тест из Тесты Цикл
		...
		|НаборТестов.Добавить(""%1"", Параметры, 
		|						""%2"");";
		...
		Для Каждого СтрокаДерева из Дерево.Строки.НайтиСтроки(Отбор, Истина) Цикл
			...
			Если СведенияОКоманде.ТипУзла = "Независимый" Тогда
				...
				ИначеЕсли СтрокаДерева.ИдентификаторКоманды = ИдОстаткиПоРегиструБухгалтерии Тогда
					...
					ТекстШаг = СтрШаблон("Шаг = ""Проверка остатков по регистру бухгалтерии %1"";", СтрокаДерева.ТипОбъекта);
					...
					Для каждого СтрокаОтбора из ДанныеПодстановки.Отбор Цикл
						
						Если СтрокаОтбора.Использовать = Ложь Тогда
							Продолжить;
						КонецЕсли;
						...
						Если ЗначениеЗаполнено(СтрокаОтбора.Валюта) Тогда
							МассивТекстовОтбораРезультата.Добавить(
							СтрШаблон("ОтборРезультат.Вставить(""Валюта"", %1.%2);", 
							ИмяСтуркутрыВходящихДанных, ПолучитьИдентификаторПредставленияПоСсылке(СтрокаОтбора.Валюта)));
						КонецЕсли;
						
						Для тт = 1 по 3 Цикл
							
							ИмяСубконто = "Субконто" + тт;
							ЗначениеСубконто = СтрокаОтбора[ИмяСубконто];
							...
							Если ЗначениеЗаполнено(ЗначениеСубконто) Тогда
								МассивТекстовОтбораРезультата.Добавить(
									СтрШаблон("ОтборРезультат.Вставить(""%1"", %2.%3);",
										...
										ПолучитьИдентификаторПредставленияПоСсылке(ЗначениеСубконто)));
							КонецЕсли;
							
						КонецЦикла;
						
						Если ЗначениеЗаполнено(СтрокаОтбора.ВалютнаяСуммаОстаток) Тогда
							
							 ПредставлениеЗначения = ПолучитьПредставлениеОбъектаКоманды(
							 	СтрокаОтбора.ВалютнаяСуммаОстаток, ИмяСтуркутрыВходящихДанных);
								
							МассивТекстовРесурсовРезультата.Добавить(
								СтрШаблон("ОтборРесурсов.Вставить(""ВалютнаяСуммаОстаток"", %1);", ПредставлениеЗначения.СтроковоеПредставление));

						КонецЕсли;
						
						Если ЗначениеЗаполнено(СтрокаОтбора.СуммаОстаток) Тогда
							
							ПредставлениеЗначения = ПолучитьПредставлениеОбъектаКоманды(
							 	СтрокаОтбора.СуммаОстаток, ИмяСтуркутрыВходящихДанных);
								
							МассивТекстовРесурсовРезультата.Добавить(
								СтрШаблон("ОтборРесурсов.Вставить(""СуммаОстаток"", %1);", ПредставлениеЗначения.СтроковоеПредставление));
					...
					Счета = ДанныеПодстановки.Отбор.ВыгрузитьКолонку("Счет");
					Счета = СтрСоединить(Счета, ", ");
				...
				//
				//                                   			
				//		Шаг = "Проверка остатков регистра ВУ_Требования после проведения Корректировки Долга";
				//
				//РаботаСРегистрами.ОстаткиПоРегиструНакопления("ВУ_Требования",  ИсходныеДанные.ДатаКонтроля, ИсходныеДанные.Портфель); 
				//
				//Фильтр = новый Структура();	
				//Фильтр.Вставить("Контрагент", ИсходныеДанные.Контрагент);
				//Фильтр.Вставить("Договор",  ИсходныеДанные.Контрагент_ДоговорСПокупателем_1);
				//                      	
				//Ресурсы = новый Структура();
				//Ресурсы.Вставить("ВалютнаяСуммаОстаток", -1000);
				//
				//РаботаСРегистрами.ПроверитьРезультат(Фильтр, Ресурсы, Ожидаем, Шаг);
