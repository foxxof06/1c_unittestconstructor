...
Процедура СгенерироватьТекстыМодуля()
	...
	Для Каждого Тест из Тесты Цикл
		...
		|НаборТестов.Добавить(""ТестДолжен_%1"", Параметры, 
		|						""%2"");";
	...
	Для каждого Тест из Тесты Цикл
		...
		ТекстФункций.ДобавитьСтроку(СтрШаблон("Процедура %1(Параметры)", Тест.ИмяТеста));
...
Процедура ОбновитьДанныеКВыгрузкеНаСервере()
	...
	Для каждого СтрокаШага из ШагиСценария Цикл
		...
		Если ЭтоСсылочныйТип(СтрокаШага.ОбъектКоманды) Тогда
			
			НоваяСтрока = ДанныеВыгрузкиОбъектов.Добавить();
			НоваяСтрока.Ссылка = СтрокаШага.ОбъектКоманды;
			НоваяСтрока.ИмяПеременной = СтрокаШага.ИмяПеременной;
...
&НаКлиенте
Процедура СкрытьКолонки(Команда)
