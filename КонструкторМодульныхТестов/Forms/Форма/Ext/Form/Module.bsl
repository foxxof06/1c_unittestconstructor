...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.Заголовок = ВерсияОбработки;
	...
	ЗаполнитьТаблицуМетодовОбъектов(ИмяКонфигурации);
	
	ЗаполнитьТаблицуИсключаемыхИзВыгрузкиОбъектов(ИмяКонфигурации);
...
#Область СобытияТаблицыТесты


&НаКлиенте
Процедура ПерезаполнитьСписокВыбораТестов()

	ТестСписокВыбора = Элементы.Тест.СписокВыбора;
	
	ТестСписокВыбора.Очистить();
	
	Для Каждого СтрокаТеста Из Тесты Цикл
		
		ТестСписокВыбора.Добавить(СтрокаТеста.Идентификатор, СтрокаТеста.ИмяТеста);
...
&НаКлиенте
Процедура ТестОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	ВТаблицуЗначенийНаСервере(ТекущийИдентификаторТеста);
	
	ТекущийИдентификаторТеста = ВыбранноеЗначение;
	
	ВДеревоЗначенийНаСервере(ВыбранноеЗначение);
...
Процедура ТестыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Или Копирование Тогда		
		...
		Элемент.ТекущиеДанные.ИмяМакетаИсходныхДанных = "ИсходныеДанные1";
		Элемент.ТекущиеДанные.ИмяМакетаСценария = "Сценарий1_scenario";
	КонецЕсли;  	
	
КонецПроцедуры
         
&НаСервере
Процедура СкопироватьТестНаСервере()
	 	
	ТекущаяСтрока = Тесты.НайтиПоИдентификатору(Элементы.Тесты.ТекущаяСтрока);
	...
	НоваяСтрокаТеста = Тесты.Добавить();
	
	ЗаполнитьЗначенияСвойств(НоваяСтрокаТеста, ТекущаяСтрока);
	
	НоваяСтрокаТеста.Идентификатор = Новый УникальныйИдентификатор();
	
	НоваяСтрокаТеста.ИмяТеста = НоваяСтрокаТеста.ИмяТеста + "_копия";	
	
	Отбор = Новый Структура("ИдентификаторТеста", ТекущаяСтрока.Идентификатор);
		
	СтрокиПоКопируемогоТеста = ШагиСценария.НайтиСтроки(Отбор);
	
	Для Каждого СтрокаШагаКопируемогоТеста Из СтрокиПоКопируемогоТеста Цикл
		
		НоваяСтрока = ШагиСценария.Добавить();
		
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаШагаКопируемогоТеста);
		
		НоваяСтрока.ИдентификаторТеста = НоваяСтрокаТеста.Идентификатор;				
	...
	ИначеЕсли ТекущиеДанные.ИдентификаторКоманды = "СоздатьДокумент" Или ТекущиеДанные.ИдентификаторКоманды = "СоздатьСправочник" Тогда
		...
		ОткрытьФорму("ВнешняяОбработка.КонструкторМодульныхТестов.Форма.ФормаРеквизитовОбъектов",
			Новый Структура(
				...
				ТекущиеДанные.ПараметрыХранилище)
				, ЭтотОбъект);   	
	ИначеЕсли ТекущиеДанные.ИдентификаторКоманды = "ПоискСтрокТаблицы"Тогда
		
		СтандартнаяОбработка = Ложь; 
		
		ОткрытьФорму("ВнешняяОбработка.КонструкторМодульныхТестов.Форма.ФормаПоискаСтрокТаблицы",
			Новый Структура(
				"СсылкаНаОбъект, ИмяТабличнойЧасти, Хранилище",
				ОбъектГлавногоУзла(),
				ТекущиеДанные.ТипОбъекта,
				ТекущиеДанные.ПараметрыХранилище)
				, ЭтотОбъект);   				
...
&НаКлиенте
Процедура ДеревоЩагиСценарияПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
Процедура ОбработатьВыбранныеФайлыНаборТестов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
    ...
    Для каждого ПереданныйФайл Из ПомещенныеФайлы Цикл
		...
		Если Тесты.Количество() > 0 Тогда
			ПерезаполнитьСписокВыбораТестов();
			ЭтотОбъект.Тест = Тесты[0].Идентификатор;
			ТекущийИдентификаторТеста = Тесты[0].Идентификатор;
		КонецЕсли;
...
Функция ДанныеДляСохраненияНабораТестовНаСервере()
	...
	ДанныеДляЗаписи.Вставить("КаталогВыгрузки", Объект.КаталогВыгрузки);
...
Функция ПолучитьПредставлениеОбъектаКоманды(Значение, ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных)
	...
	Если ТипТекущегоЗначения = Тип("Число") Тогда
		...
		ПредставлениеСценария = ПредставлениеЗначения;
	...
	ИначеЕсли ТипТекущегоЗначения = Тип("Строка") Тогда
		...
		ПредставлениеСценария = Значение;
	...
	ИначеЕсли ТипТекущегоЗначения = Тип("Дата") Тогда
		...
		ПредставлениеСценария = Формат(Значение, "ДЛФ=DD");
	...
	ИначеЕсли ТипТекущегоЗначения = Тип("Неопределено") Тогда
		...
		ПредставлениеСценария = "<Неопределено>"
	...
	ИначеЕсли ТипТекущегоЗначения = Тип("ПланСчетовСсылка.Управленческий")
		...
		ПредставлениеСценария = Строка(Значение);
	...
	ИначеЕсли ЭтоСсылочныйТип(Значение) Тогда
		...
		Если Не ЗначениеЗаполнено(Значение) Тогда
			...
			ИмяМета = Значение.Метаданные().Имя;
			
			Если Метаданные.Справочники.Содержит(Мета) Тогда 
				Парам1 = "Справочники." + ИмяМета;
			ИначеЕсли Метаданные.Документы.Содержит(Мета)  Тогда
				Парам1 = "Документы." + ИмяМета;
			КонецЕсли;
			
			Парам2 = "ПустаяСсылка()";  
		ИначеЕсли Метаданные.Справочники.Содержит(Значение.Метаданные()) И Значение.Предопределенный Тогда
	...
	Возврат Новый Структура("ПредставлениеЗначения, СтроковоеПредставление, ПредставлениеСценария", ПредставлениеЗначения, СтроковоеПредставление, ПредставлениеСценария);
...
Процедура ОбновитьДанныеКВыгрузкеНаСервере()
	...
	Для тт = 0 По МассивИспользованныхИмен.ВГраница() Цикл
		
		МассивИспользованныхИмен[тт] = НРег(МассивИспользованныхИмен[тт]);
		
	КонецЦикла;
	
		
	ИсключаемыеРеквизиты = ИсключаемыеИзВыгрузкиРеквизитыОбъектов.Выгрузить().ВыгрузитьКолонку("ИмяРеквизита");
	
	
	ТипыДокументовВыгружаемыхСДвижениями = ТипыДокументовВыгружаемыхСДвижениями(Метаданные.Имя);
	...
	Для Каждого СтрокаТаблицы из ТаблицаИспользованныхСсылочныхОбъектов Цикл
		...
		НоваяСтрока.ВыгружатьСДокументомЕгоДвижения = ТипыДокументовВыгружаемыхСДвижениями.Найти(ТипЗнч(НоваяСтрока.Ссылка)) <> Неопределено;
		
		
		
		МассивОбъектов = Новый Массив;
		
		ПолучитьОбъектыРекурсивно(НоваяСтрока.Ссылка, МассивОбъектов, ИсключаемыеРеквизиты,ТипыДокументовВыгружаемыхСДвижениями, 0, 2); 
...
&НаСервере
Процедура ПолучитьОбъектыРекурсивно(СсылкаНаОбъект, МассивОбъектов,  ИсключаемыеРеквизиты, ТипыДокументовВыгружаемыхСДвижениями, ТекущийУровеньРекурсии, МаксУровеньРекурсии) 
	
	Если ЭтоПеречисление(СсылкаНаОбъект) Или НЕ ЭтоСсылочныйТип(СсылкаНаОбъект) Или Не ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЭтоДокумент Или ЭтоСправочник Тогда
		...
		Для каждого РеквизитОбъекта из МетаданныеСсылки.Реквизиты Цикл
				...
				Если ИсключаемыеРеквизиты.Найти(РеквизитОбъекта.Имя) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
			...
			ПолучитьОбъектыРекурсивно(ЗначениеРеквизита, МассивОбъектов, ИсключаемыеРеквизиты, ТипыДокументовВыгружаемыхСДвижениями, ТекущийУровеньРекурсии + 1, МаксУровеньРекурсии);
			
		КонецЦикла;
		
		Для каждого ТабличнаяЧастьОбъекта из МетаданныеСсылки.ТабличныеЧасти Цикл
			
			Если ИсключаемыеРеквизиты.Найти(ТабличнаяЧастьОбъекта.Имя) <> Неопределено  Тогда
				Продолжить;
			КонецЕсли;
		...
		Если ЭтоДокумент И ТипыДокументовВыгружаемыхСДвижениями.Найти(ТипЗнч(СсылкаНаОбъект)) <> Неопределено Тогда
								
			ОбъектДокумента = СсылкаНаОбъект.ПолучитьОбъект();
			
			Для Каждого Движение Из ОбъектДокумента.Движения Цикл
				...
				Для Каждого СтрокаДвижений Из ТаблицаДвижений Цикл
					...
					Для Каждого Колонка из ТаблицаДвижений.Колонки Цикл
						...
						ЗначениеРеквизита = СтрокаДвижений[Колонка.Имя];
						...
						Если ЗначениеРеквизита = СсылкаНаОбъект Тогда
							Продолжить;
						КонецЕсли;
...
Функция СтурктураДляЗаписи()
	...
	ДанныеДляЗаписи.Вставить("ИсключаемыеИзВыгрузкиРеквизитыОбъектов", ИсключаемыеИзВыгрузкиРеквизитыОбъектов.Выгрузить());
...
Процедура ПрочитатьФайлНаборТестовНаСервере(Адрес, ИмяФайла) 
	...
	Объект.КаталогВыгрузки = ДанныеКЗагрузке.КаталогВыгрузки;
...
Процедура ЗаполнитьТаблицуДоступныхКоманд()
	...
	НоваяСтрока.Представление = "Создать справочник";
	...
	НоваяСтрока = ДостпныеКоманды.Добавить();
	НоваяСтрока.ИдентификаторКоманды = "ПоискСтрокТаблицы";
	НоваяСтрока.Представление = "Поиск строк в таблице объекта";
	НоваяСтрока.ОграничениеТипа = "ТабличныеЧастиОбъекта";
	НоваяСтрока.Вид = "Утверждение";
	НоваяСтрока.ТипУзла = "Подчиненный";
...
&НаСервере 
Процедура ЗаполнитьТаблицуМетодовОбъектов(ИмяКонфигурации)
	   	
	ШаблонИмяМакета = ИмяКонфигурации + "_ПроцедурыФункцииОбъектов";
	
	ЕстьМакет = РеквизитФормыВЗначение("Объект").Метаданные().Макеты.Найти(ШаблонИмяМакета) <> Неопределено;
	
	Если Не ЕстьМакет Тогда
		Возврат;
	КонецЕсли;
		
	МакетТаблицыМетодов = РеквизитФормыВЗначение("Объект").ПолучитьМакет(ШаблонИмяМакета); 
...
&НаСервере 
Процедура ЗаполнитьТаблицуИсключаемыхИзВыгрузкиОбъектов(ИмяКонфигурации)
	   	
	ШаблонИмяМакета = ИмяКонфигурации + "_ИсключаемыеИзВыгрузкиРеквизитыОбъектов";
	...
	Макет = РеквизитФормыВЗначение("Объект").ПолучитьМакет(ШаблонИмяМакета); 
	...
	ИсключаемыеИзВыгрузкиРеквизитыОбъектов.Загрузить(ПостроительОтчета.Результат.Выгрузить());
	
КонецПроцедуры

&НаСервере 
Функция ТипыДокументовВыгружаемыхСДвижениями(ИмяКонфигурации)
	   	
	ШаблонИмяМакета = ИмяКонфигурации + "_ДокументыВыгружаемыеСДвижениями";
	...
	Для каждого ТипСтрокой Из ПостроительОтчета.Результат.Выгрузить().ВыгрузитьКолонку("ТипДокумента")Цикл
		
		Попытка
			МассивТипов.Добавить(Тип(ТипСтрокой));
		Исключение
			
			ТекстСообщения = СтрШаблон("Не удалось преобразовать значение '%1' в Тип!", ТипСтрокой);
...
Процедура СгенерироватьТекстыМодуля()
	...
	Для Каждого ТестОбработки из Тесты Цикл 		
		...
		Шаблон = "Параметры = ПараметрыСценария(""%1"");
		|НаборТестов.Добавить(""ТестДолжен_%2"", Параметры, ""%3"");";   		
		
		Текст = СтрШаблон(Шаблон, ТестОбработки.ИмяМакетаСценария, ТестОбработки.ИмяТеста, ТестОбработки.Описание);
	...
	ТекстСценария = Новый ТекстовыйДокумент;
	...
	Для каждого ТестОбработки из Тесты Цикл
		...
		Отбор = Новый Структура("ИдентификаторТеста, ИдРодителя", ИдентификаторТеста, "");
		
		
		ТекстФункций.ДобавитьСтроку(""); 
		ТекстФункций.ДобавитьСтроку(СтрШаблон("Процедура ТестДолжен_%1(Параметры) Экспорт", ТестОбработки.ИмяТеста));	        
		ТекстФункций.ДобавитьСтроку("");		
		ТекстФункций.ДобавитьСтроку(СтрШаблон("ЗаполнитьСтруктуруВходящихДанныхИзМакета(""%1"");", ТестОбработки.ИмяМакетаИсходныхДанных));
		ТекстФункций.ДобавитьСтроку("");
		
		
		Для Каждого СтрокаДерева из ШагиСценария.НайтиСтроки(Отбор) Цикл
			...
			Если СведенияОКоманде.ТипУзла = "Независимый" Тогда
				...
				Если СтрокаДерева.ИдентификаторКоманды = "УстановитьЗначениеКонстанты" Тогда
					...
					ТекстУстановитьЗначениеКонстанты = СтрШаблон("Константы.%1.Установить(%2);", СтрокаДерева.ТипОбъекта, ПредставлениеЗначения.СтроковоеПредставление);
					
					ТекстФункций.ДобавитьСтроку("");					
					ТекстФункций.ДобавитьСтроку(ТекстУстановитьЗначениеКонстанты); 					
					ТекстФункций.ДобавитьСтроку("");
					
					ТекстШагаСценария = СтрШаблон("Установлено значение константы '%1': %2 -%3", СтрокаДерева.ОбъектКоманды, Символы.ПС, ПредставлениеЗначения.ПредставлениеСценария); 					
					ТекстСценария.ДобавитьСтроку(ТекстШагаСценария);
				...
				ИначеЕсли СтрокаДерева.Команда = "ПолучитьДокументИзСтруктурыИПровести" Тогда
					...
					ТекстШагаСценария = СтрШаблон("Проведен документ '%1' от %2", ТипЗнч(СтрокаДерева.ОбъектКоманды), СтрокаДерева.ОбъектКоманды.Дата); 					
					ТекстСценария.ДобавитьСтроку(ТекстШагаСценария);				
				...
				ИначеЕсли СтрокаДерева.ИдентификаторКоманды = ИдДанныеРегистраСведений Тогда
					...
					ТекстШагаСценария = СтрШаблон("ТекстКоманды_ДанныеРегистраСведений"); 					
					ТекстСценария.ДобавитьСтроку(ТекстШагаСценария);	
				...
				ИначеЕсли СтрокаДерева.ИдентификаторКоманды = ИдОборотыПоРегиструНакопления Тогда
					...
					ДанныеПодстановки = ПолучитьДанныеИзХранилища(СтрокаДерева.ПараметрыХранилище, ИдОборотыПоРегиструНакопления);
					
					ОбщийОтборПоПортфелю = Ложь;
					
					ПредставлениеНачалоПериода 	= ПолучитьПредставлениеОбъектаКоманды(ДанныеПодстановки.ПериодОборотов.ДатаНачала, ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных);
					ПредставлениеКонецПериода 	= ПолучитьПредставлениеОбъектаКоманды(ДанныеПодстановки.ПериодОборотов.ДатаОкончания, ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных);
					
					Если ДанныеПодстановки.Портфели.Количество() = 1 Тогда
						ПортфельПредставление = ПолучитьПредставлениеОбъектаКоманды(ДанныеПодстановки.Портфели[0], ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных);
						ПортфельСтроковоеПредставление = ПортфельПредставление.СтроковоеПредставление;
						ОбщийОтборПоПортфелю = Истина;
						
						ТекстПолученияОстатков = "РаботаСРегистрами.ОборотыПоРегиструНакопленияЗаПериод(""%1"", %2, %3, %4);";
						ТекстСПараметрами = СтрШаблон(ТекстПолученияОстатков, СтрокаДерева.ТипОбъекта, ПредставлениеНачалоПериода.СтроковоеПредставление, ПредставлениеКонецПериода.СтроковоеПредставление, ПортфельСтроковоеПредставление);
	
					Иначе
						ТекстПолученияОстатков = "РаботаСРегистрами.ОборотыПоРегиструНакопленияЗаПериод(""%1"", %2, %3);";
						ТекстСПараметрами = СтрШаблон(ТекстПолученияОстатков, СтрокаДерева.ТипОбъекта, ПредставлениеНачалоПериода.СтроковоеПредставление, ПредставлениеКонецПериода.СтроковоеПредставление);
					КонецЕсли;
					...
					ТекстШагаСценария = СтрШаблон("Получаем обороты по регистру накопления '%1' за период c %2 по %3", СтрокаДерева.ТипОбъекта, ПредставлениеНачалоПериода.ПредставлениеСценария, ПредставлениеКонецПериода.ПредставлениеСценария);
					ТекстСценария.ДобавитьСтроку(ТекстШагаСценария); 
					ТекстСценария.ДобавитьСтроку("- В разрезе:");
					                   						
					ТекстФункций.ДобавитьСтроку(ТекстСПараметрами);  
						
					                                            					
					Для Каждого СтрокаТаблицы Из ДанныеПодстановки.ТаблицаДанных Цикл
						
						ТекстФункций.ДобавитьСтроку("");
						ТекстФункций.ДобавитьСтроку(ТекстФильтр);
						
						Для каждого Измерение из ДанныеПодстановки.ИзмеренияДляОтбора Цикл
							
							Если ОбщийОтборПоПортфелю И Измерение = "Портфель" Тогда
								Продолжить;
							КонецЕсли;	
							
							ЗначениеИзмерения = СтрокаТаблицы[Измерение];
							
							ПредставлениеЗначения = ПолучитьПредставлениеОбъектаКоманды(ЗначениеИзмерения, ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных);
							
							ТекстФункций.ДобавитьСтроку(
							СтрШаблон(ТекстФильтр1, Измерение, ПредставлениеЗначения.СтроковоеПредставление)
							);
							
							ТекстСценария.ДобавитьСтроку(СтрШаблон("-- %1: %2", Измерение, ПредставлениеЗначения.ПредставлениеСценария));
						...
						Для каждого Ресурс из ДанныеПодстановки.Ресурсы Цикл
							...
							ЗначениеРесурса = СтрокаТаблицы[Ресурс];
							
							Если Не ЗначениеЗаполнено(ЗначениеРесурса) Тогда
								Продолжить;
							КонецЕсли;
							                							
							ПредставлениеЗначения = ПолучитьПредставлениеОбъектаКоманды(ЗначениеРесурса, ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных);
							
							ТекстФункций.ДобавитьСтроку(СтрШаблон(ТекстРесурсы1, Ресурс, ПредставлениеЗначения.СтроковоеПредставление));
							
						КонецЦикла;
						
						ТекстФункций.ДобавитьСтроку("");
						ТекстФункций.ДобавитьСтроку("РаботаСРегистрами.ПроверитьРезультат(Фильтр, Ресурсы, Ожидаем, Шаг);");
						ТекстФункций.ДобавитьСтроку(""); 					
				...
				ИначеЕсли СтрокаДерева.ИдентификаторКоманды = ИдОстаткиПоРегиструНакопления Тогда
					...
					ОбщийОтборПоПортфелю = Ложь;
					ПредставлениеПериода = СтрШаблон("Дата('%1')", Формат(ДанныеПодстановки.Период, "ДФ=ггггММддЧЧммсс"));
					...
					Если ДанныеПодстановки.Портфели.Количество() = 1 Тогда
						...
						ТекстСПараметрами = СтрШаблон(ТекстПолученияОстатков, СтрокаДерева.ТипОбъекта, ПредставлениеПериода, ПортфельСтроковоеПредставление);
	
					Иначе
						ТекстПолученияОстатков = "РаботаСРегистрами.ОстаткиПоРегиструНакопления(""%1"",  %2);";
						ТекстСПараметрами = СтрШаблон(ТекстПолученияОстатков, СтрокаДерева.ТипОбъекта, ПредставлениеПериода);
					КонецЕсли;
					...
					Для Каждого СтрокаТаблицы Из ДанныеПодстановки.ТаблицаДанных Цикл
						...
						Для каждого Измерение из ДанныеПодстановки.ИзмеренияДляОтбора Цикл
							...
							ТекстФункций.ДобавитьСтроку(СтрШаблон(ТекстФильтр1, Измерение, ПредставлениеЗначения.СтроковоеПредставление));
			...
			ИначеЕсли СведенияОКоманде.ТипУзла = "Родительский" Тогда 
				...
				ИначеЕсли СтрокаДерева.Команда = "СоздатьДокумент" Тогда
					...
					ТекстШаг = СтрШаблон("Шаг = ""Создать документ  '%1'"";", ИмяМетаданного);
			        ТекстФункций.ДобавитьСтроку("");
					ТекстФункций.ДобавитьСтроку(ТекстШаг);
					
					ТекстФункций.ДобавитьСтроку(СтрШаблон("%1 = Документы.%2.СоздатьДокумент();", Контекст.ПеременнаяОбъекта, ИмяМетаданного));
					...
					Для Каждого	ТаблицаДанных Из ДанныеОбъекта.ТаблицыДанных Цикл
						...
						Для каждого СтрокаДанныхТаблицы из ТаблицаДанных.ДанныеТаблицы Цикл
							...
							Для Каждого КолонкаТаблицы Из ТаблицаДанных.ДанныеТаблицы.Колонки Цикл 
								...
								Если Не ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
									Продолжить;
								ИначеЕсли ТипЗнч(ЗначениеРеквизита) = Тип("Булево") И ЗначениеРеквизита = Ложь Тогда
									Продолжить;
								КонецЕсли;
				...
				Для каждого ПодчиненнаяСтрока из ШагиСценария.НайтиСтроки(Новый Структура("ИдентификаторТеста, ИдРодителя", ИдентификаторТеста, СтрокаДерева.Ид)) Цикл
					...
					ИначеЕсли ПодчиненнаяСтрока.ИдентификаторКоманды = "ПоискСтрокТаблицы" Тогда
						
						//Отбор = Новый Структура("Контрагент, ДоговорКонтрагента, ДокументРасчетов, ГрафикПогашенияЗадолженности");	
						ДанныеОбъекта = ПолучитьДанныеИзХранилища(ПодчиненнаяСтрока.ПараметрыХранилище, "ПоискСтрокТаблицы");
						
						ТаблицаОтбора = ДанныеОбъекта.ТаблицаОтбора;						
						КолонкиОтбора = ДанныеОбъекта.КолонкиОтбора;						
						ИтоговыеКолонки = ДанныеОбъекта.ИтоговыеКолонки; 						
						КлючОтбора = СтрСоединить(КолонкиОтбора, ", ");
						ИмяТаблицыОбъекта = ДанныеОбъекта.ИмяТаблицыОбъекта;
						ТипОбъекта = ТипЗнч(ДанныеОбъекта.ОбъектБД);
					
						
						СтрокаОтбор = СтрШаблон("Отбор = Новый Структура(""%1"");", КлючОтбора);
						
						ТекстФункций.ДобавитьСтроку(СтрокаОтбор);
					                                   						
												
						Для каждого СтрокаОтбора Из ТаблицаОтбора Цикл
							
							СтрокаКонструкторОтбора = "";

							 							
							Для Каждого КолонкаОтбора Из  КолонкиОтбора Цикл
								
								ЗначениеОтбора = СтрокаОтбора[КолонкаОтбора];
								
								ПредставлениеОбъекта = ПолучитьПредставлениеОбъектаКоманды(ЗначениеОтбора, ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных);
								
								СтрокаКонструкторОтбора = СтрокаКонструкторОтбора + СтрШаблон("Отбор.%1 = %2;", КолонкаОтбора, ПредставлениеОбъекта.СтроковоеПредставление) + Символы.ПС;						
							
							КонецЦикла;
							
							СтрокаПоиска = СтрШаблон("НайденныеСтроки = %1.%2.НайтиСтроки(Отбор);", Контекст.ПеременнаяОбъекта, ИмяТаблицыОбъекта);
							
							ТекстОтбора = СтрШаблон("Количество строк таблицы '%1' документа '%2'", ИмяТаблицыОбъекта, ТипОбъекта);
							
							СтрокаОжиданияКоличества = СтрШаблон("Ожидаем.Что(НайденныеСтроки.Количество(), ""%1"").Равно(%2);", ТекстОтбора, СтрокаОтбора.КоличествоНайденныхСтрок);
							
							ТекстФункций.ДобавитьСтроку(СтрокаКонструкторОтбора);
							ТекстФункций.ДобавитьСтроку(СтрокаПоиска);
							ТекстФункций.ДобавитьСтроку("");
							ТекстФункций.ДобавитьСтроку(СтрокаОжиданияКоличества);
							ТекстФункций.ДобавитьСтроку("");  
							
							СтрокаОжиданияИтогов = "";
							
							Если СтрокаОтбора.КоличествоНайденныхСтрок = 1 Тогда
								
								Для Каждого КолонкаИтогов Из ИтоговыеКолонки Цикл
									
									ЗначениеИтога = СтрокаОтбора[КолонкаИтогов];
									ПредставлениеЗначения = ПолучитьПредставлениеОбъектаКоманды(ЗначениеИтога, ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных);
									                              								
									СтрокаОжиданияИтогов = СтрокаОжиданияИтогов + СтрШаблон("Ожидаем.Что(НайденныеСтроки[0].%1, ТекстОтбора).Равно(%2);", КолонкаИтогов, ПредставлениеЗначения.СтроковоеПредставление) + Символы.ПС;
								
								КонецЦикла;
								
							ИначеЕсли СтрокаОтбора.КоличествоНайденныхСтрок > 1 Тогда
								
								СтрокаКолонокИтогов = СтрСоединить(ИтоговыеКолонки, ", ");
								
								СтрокаОжиданияИтогов = СтрШаблон("ТаблицаИтогов = %1.%2.Скопировать(НайденныеСтроки, ""%3"")", Контекст.ПеременнаяОбъекта, ИмяТаблицыОбъекта, СтрокаКолонокИтогов);
								
								СтрокаОжиданияИтогов = СтрокаОжиданияИтогов + Символы.ПС + Символы.ПС;
									                                    																
								Для Каждого КолонкаИтогов Из ИтоговыеКолонки Цикл
									
									СтрокаОжиданияИтогов = СтрокаОжиданияИтогов + СтрШаблон("%1 = ТаблицаИтогов.Итог(""%1"");", КолонкаИтогов) + Символы.ПС;
									
									ТекстОжидания = СтрШаблон("Итог по найденным строкам колонки '%1'", КолонкаИтогов);
									
									ЗначениеИтога  = СтрокаОтбора[КолонкаИтогов];
									
									ПредставлениеЗначения = ПолучитьПредставлениеОбъектаКоманды(ЗначениеИтога, ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных);
									
									СтрокаОжиданияИтогов = СтрокаОжиданияИтогов + СтрШаблон("Ожидаем.Что(%1, ""%2"").Равно(%3);", КолонкаИтогов, ТекстОжидания, ПредставлениеЗначения.СтроковоеПредставление) + Символы.ПС + Символы.ПС;
							...
							ТекстФункций.ДобавитьСтроку("");
							ТекстФункций.ДобавитьСтроку(СтрокаОжиданияИтогов);
...
Функция ТекстКоманды_ДанныеРегистраСведений(ИмяСтуркутрыВходящихДанных, СтрокаДерева, ДанныеРегистра, МассивИспользованныхИменПеременных)
	...
	Для каждого Запись Из ДанныеРегистра.ТаблицаДанных Цикл
		...
		Для каждого КолонкаЗаписи Из ДанныеРегистра.ТаблицаДанных.Колонки Цикл
			...
			Если ТипЗнч(Значение) = Тип("Булево") И Значение = Ложь
					Или ТипЗнч(Значение) = Тип("Число") И Значение = 0
					Или ТипЗнч(Значение) = Тип("Строка") И ПустаяСтрока(Значение)  Тогда
				Продолжить;
			КонецЕсли; 
...
Процедура РежимОтладкиПриИзменении(Элемент)
	...
	ВыбранноеЗначение = Объект.РежимОтладки;
	
	Элементы.ДеревоЩагиСценарияИд.Видимость = ВыбранноеЗначение;
	Элементы.ДеревоЩагиСценарияИдРодителя.Видимость = ВыбранноеЗначение;
	
КонецПроцедуры

&НаКлиенте
Процедура ТестыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ПерезаполнитьСписокВыбораТестов();
КонецПроцедуры
