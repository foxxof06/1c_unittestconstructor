...
Процедура ТестНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Для Каждого СтрокаТеста Из Тесты Цикл
		...
		Элемент.СписокВыбора.Добавить(СтрокаТеста.Идентификатор, СтрокаТеста.ИмяТеста);
...
Функция ПолучитьДанныеИзХранилища(ХранилищеДанных, ИдКоманды)
	...
	Если ИдКоманды = ИдОстаткиПоРегиструНакопления Или
		...
		СтруктураСериализации = Новый Структура("ИмяРегистра, Портфель, Период, ПериодОборотов, Измерения, Ресурсы, ТаблицаДанных");
		...
		Для Каждого СтрокаДерева из ШагиСценария.НайтиСтроки(Отбор) Цикл
			...
			Если СведенияОКоманде.ТипУзла = "Независимый" Тогда
				...
				ИначеЕсли СтрокаДерева.ИдентификаторКоманды = ИдОборотыПоРегиструНакопления Тогда
					...
					ПредставлениеПортфеля = ПолучитьПредставлениеОбъектаКоманды(ДанныеПодстановки.Портфель, ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных);
					...
					ТекстФункций.ДобавитьСтроку(
						...
						СтрокаДерева.ТипОбъекта, ПредставлениеНачалоПериода.СтроковоеПредставление,ПредставлениеКонецПериода.СтроковоеПредставление , ПредставлениеПортфеля.СтроковоеПредставление)
						);  
						...
						Для Каждого СтрокаТаблицы Из ДанныеПодстановки.ТаблицаДанных Цикл
							...
							Для каждого Измерение из ДанныеПодстановки.Измерения Цикл
							...
							Для каждого Ресурс из ДанныеПодстановки.Ресурсы Цикл
								...
								ТекстФункций.ДобавитьСтроку(
									СтрШаблон(ТекстРесурсы1, Ресурс, ПредставлениеЗначения.СтроковоеПредставление)
									);
				...
				ИначеЕсли СтрокаДерева.ИдентификаторКоманды = ИдОстаткиПоРегиструНакопления Тогда
					...
					ТекстФункций.ДобавитьСтроку(
						СтрШаблон(ТекстПолученияОстатков,
						СтрокаДерева.ТипОбъекта, ПредставлениеПериода, ПредставлениеПортфеля.СтроковоеПредставление)
						);  
