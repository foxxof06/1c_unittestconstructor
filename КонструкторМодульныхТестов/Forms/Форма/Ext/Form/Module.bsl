		...
		Для Каждого СтрокаДерева из ШагиСценария.НайтиСтроки(Отбор) Цикл
			...
			ИначеЕсли СведенияОКоманде.ТипУзла = "Родительский" Тогда 
				...
				ИначеЕсли СтрокаДерева.Команда = "СоздатьДокумент" Тогда
					...
					Текст = СтрШаблон("%1.Записать();", Контекст.ПеременнаяОбъекта);
					ТекстФункций.ДобавитьСтроку(Текст);
				...
				Для каждого ПодчиненнаяСтрока из ШагиСценария.НайтиСтроки(Новый Структура("ИдРодителя", СтрокаДерева.Ид)) Цикл
