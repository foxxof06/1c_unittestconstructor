		...
		Для Каждого СтрокаДерева из Дерево.Строки.НайтиСтроки(Отбор, Истина) Цикл
			...
			Если СведенияОКоманде.ТипУзла = "Независимый" Тогда
				...
				ИначеЕсли СтрокаДерева.ИдентификаторКоманды = ИдОборотыПоРегиструНакопления Тогда
					...
					ТекстПолученияОборотов = "РаботаСРегистрами.ОборотыПоРегиструНакопленияЗаПериод(""%1"",  %2, %3, %4);";
					...
					ПредставлениеПортфеля = ПолучитьПредставлениеОбъектаКоманды(ДанныеПараметры.Портфель, ИмяСтуркутрыВходящихДанных, МассивИспользованныхИменПеременных);
					...
					ТекстФункций.ДобавитьСтроку(
					СтрШаблон(ТекстПолученияОборотов,
